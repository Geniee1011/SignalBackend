 ...
 *
 * Each market is derived from the (access-scoped) signal set:
 *   - status     : Active Trade (open signal) / In Play (recent) / Standby (none)
 *   - conviction : 0-100, from the latest signal's conviction phase (1-4) plus a
 *                  recent-activity nudge, so it reflects real data
...
export function buildDashboard(signals: Signal[], allowedMarkets: readonly string[]): Dashboard {
  ...
  const list: DashboardMarket[] = markets.map((mkt) => {
    ...
    if (latest) {
      // Derived from real signal data: conviction phase, number of signals, whether
      // a trade is open, and recency. These real differentiators separate an active,
      // busy market from a quiet one even when phases are uniform (seeded demo data).
      const base = PHASE_PCT[clamp(latest.conviction || 1, 1, 4)] ?? 40;
      const activity = Math.min(ms.length, 6) * 5;
      const live = active ? 20 : 6;
